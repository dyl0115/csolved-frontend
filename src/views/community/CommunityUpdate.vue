    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      <div class="bg-white rounded-lg shadow-lg">
        <div class="bg-gray-800 text-white px-6 py-4 rounded-t-lg">
          <h1 class="text-xl font-semibold mb-0">글 수정하기</h1>
        </div>
        ...
        <div class="p-6">
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500">예: 유머, 고민, 취업</p>
            </div>
...
import { getCommunityPostDetail, updateCommunityPost, getCategories } from '@/api/community'
...
const hasChanges = () => {
  return (
    form.categoryId !== originalData.categoryId ||
    form.title !== originalData.title ||
    form.content !== originalData.content ||
    form.anonymous !== originalData.anonymous ||
    JSON.stringify(tags.value) !== JSON.stringify(originalData.tags)
  )
}
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.errors) {
        ...
        alert(result.message || '글 수정 중 오류가 발생했습니다.')
      }
    }
  } catch (error) {
...
onMounted(async () => {
  await Promise.all([loadCategories(), loadPostData()])
})
</script>
